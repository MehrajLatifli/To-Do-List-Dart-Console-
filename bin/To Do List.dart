...
Future<void> userSingUpFunction() async {
  ...
  final loadedUsers = await deserializeFromJson<User<String, double>>('./Files/User/users.json','users');
  ...
  if (!userExists) {
    User<String, double> user = User(name, age, uuid.v4(), schoolname, password, grade);
...
Future<void> toDoCreateFunction() async {
  ...
  final loadedTodos = await deserializeFromJson< ToDo<String, DateTime>>('./Files/Todo/todos.json','todos');
  ...
  if (!todoExists) {
    ToDo<String, DateTime> todo = ToDo(uuid2.v4(),Get_idUser,title,text,DateTime.now());
    ...
    final updatedTodos = await deserializeFromJson< ToDo<String, DateTime> >('./Files/Todo/todos.json','todos');
